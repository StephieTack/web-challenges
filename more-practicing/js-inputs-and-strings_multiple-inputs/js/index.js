...
// to upperCase ist ein extra Feature von mir ;)
switchValueButton.addEventListener("click", () => {
  [firstSwitchValueInput.value, secondSwitchValueInput.value] = [
    secondSwitchValueInput.value.toUpperCase(),
    firstSwitchValueInput.value.toUpperCase(),
  ];
});

// so geht es ebenfalls:
// switchValueButton.addEventListener("click", () => {
//   const tempFirstSwitchValueInput = firstSwitchValueInput.value;  // Speichert den Wert des ersten Eingabefelds in einer temporären Variable
//   firstSwitchValueInput.value = secondSwitchValueInput.value;  // Weist den Wert des zweiten Eingabefelds dem ersten Eingabefeld zu
//   secondSwitchValueInput.value = tempFirstSwitchValueInput ;  // Weist den in der temporären Variable gespeicherten Wert dem zweiten Eingabefeld zu
// })
// --^-- write your code here --^--
